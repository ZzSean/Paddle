...
from paddle.incubate.operators import ResNetUnit
...
class TestResNetUnitAPI(unittest.TestCase):
    ...
    def build_fused_program(self,
        ...
        with fluid.program_guard(main_program, startup_program):
            x = fluid.layers.data(name='x', shape=[28, 28, 64], dtype='float16')
            resnet_unit = ResNetUnit(
                num_channels_x=64,
                num_filters=256,
                filter_size=1,
                stride=1,
    ...
    def check(self, place, use_cuda):
        ...
        with fluid.scope_guard(scope):
            ...
            for _ in range(iters):
                x = np.random.random((1, 28, 28, 64)).astype("float16")
                x_data.append(x)
