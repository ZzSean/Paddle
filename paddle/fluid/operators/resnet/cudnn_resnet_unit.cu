...
#define MALLOC_AND_GET_PTR(TR, Dtype, Place) \
  auto TR##_ptr = TR->mutable_data<Dtype>(Place);
    ...
    conv_x_op.Forward(ctx, input_x_ptr, filter_x_ptr, conv_out_x_ptr, sum_x_ptr,
                      sum_of_squares_x_ptr);
    // 2. BN
    CuDNNBNStatsFinalizeOp<T> bn_x_op = new CuDNNBNStatsFinalizeOp<T>();
    bn_x_op.Init(ctx, output_shape);
    bn_x_op.Forward(ctx, sum_x_ptr, sum_of_squares_x_ptr, saved_mean_x_ptr,
                    saved_invstd_x_ptr, running_mean_x_ptr, running_var_x_ptr,
    ...
    if (has_shortcut) {
      ...
      bn_z_op.Forward(ctz, sum_z_ptr, sum_of_squares_z_ptr, saved_mean_z_ptr,
                      saved_invstd_z_ptr, running_mean_z_ptr, running_var_z_ptr,
